...
interface IPlayer {
    ...
    fun initSubtitles(subtitles: Set<SubtitleData>, subtitle: SubtitleData?) {
        setActiveSubtitles(subtitles)
        setPreferredSubtitles(subtitle)
    }
...
data class SubtitleCue(val startTimeMs: Long, val durationMs: Long, val text: List<String>) {
    val startEpoch get() = (startTimeMs / 1000.0)
    val endEpoch get() = ((startTimeMs + durationMs) / 1000.0)
}
